...
<section class="section method-comparison" aria-labelledby="method-comparison-title">
 ...
 <div class="controls">
  ...
  <div class="control"><label>检峰指标 / Disk metric</label><select id="detection-comparison-metric"><option value="recall" selected>Recall</option><option value="precision">Precision</option><option value="high_angle_recall">High-angle recall</option><option value="position_rmse_px">Position RMSE (px)</option><option value="position_p95_px">Position P95 (px)</option><option value="sample_detection_coverage">Sample detection coverage</option><option value="false_positive_per_sample">False positives / sample</option><option value="false_negative_per_sample">False negatives / sample</option></select></div>
 </div>
 ...
 <div class="method-comparison-note" id="method-comparison-note">同一数据集、剂量和噪声条件；左图的 Top‑K 是最终取向候选中“前 K 个是否含有正确答案”，右图的 Recall/Precision 是检峰层与物理 oracle 的匹配。</div>
...
function setupSelectors(){for(const id of ["dose-noise","condition-noise","disk-noise","method-comparison-noise"]){addOptions(id,NOISE_ORDER,n=>NOISE_LABEL[n])}for(const id of ["noise-dose","condition-dose","disk-fixed-dose","method-comparison-dose"]){addOptions(id,DATA.dataset.doses,d=>d.toLocaleString()+" e⁻")}$("dose-noise").value="poisson_only";$("method-comparison-noise").value="poisson_only";$("noise-dose").value="10000";$("condition-dose").value="10000";$("disk-fixed-dose").value="10000";$("method-comparison-dose").value="10000";for(const c of DATA.cases){const o=document.createElement("option");o.value=c.id;o.textContent=c.label+" · "+c.sample_id;$("case-select").append(o)}}
...
function drawMethodComparison(){const scope=$("method-comparison-scope").value,dose=Number($("method-comparison-dose").value),noise=$("method-comparison-noise").value,metricKey=$("method-comparison-metric").value,metric=METRIC[metricKey],rows=methodComparisonRows(scope,dose,noise),series=metricSeries(rows.map(x=>x.row),metricKey);groupedBarChart($("method-comparison-chart"),rows.map(x=>METHOD_SHORT[x.method]),series,{yTitle:metric.label,fixed:metric.fixed});legend("method-comparison-legend",series);const detectionKey=$("detection-comparison-metric").value,detection=DISK_METRIC[detectionKey],detectors=Object.keys(DETECTOR_LABEL),detectorColors=["#2563eb","#ea580c","#15803d"],detectorSeries=[{name:detection.label,color:detectorColors[0],barColors:detectorColors,values:detectors.map(detector=>diskRow(scope,detector,dose,noise)[detection.field])}];groupedBarChart($("detection-comparison-chart"),["当前条件"],detectorSeries,{yTitle:detection.label,fixed:detection.fixed});legend("detection-comparison-legend",detectors.map((detector,i)=>({name:DETECTOR_LABEL[detector],color:detectorColors[i]})));renderMethodComparisonTable(rows,metricKey);$("method-comparison-note").textContent=`${scope==="study001"?"[001] 独立集":"Headline"} · ${dose.toLocaleString()} e⁻ · ${NOISE_LABEL[noise]}：左图把 ACOM 三个检峰路径与 Pyxem 放在同一分母下；右图只比较三个检峰器相对 oracle 的 ${detection.label}。`}
...
window.addEventListener("resize",()=>{clearTimeout(resizeTimer);resizeTimer=setTimeout(drawMethodComparison,120)});
</script>
</main></body></html>"""
